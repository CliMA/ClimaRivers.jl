# ----------------------------------------------------------------------------
# Script to create basin mappings to grid points JSON file for era5 data
...
"""
function find_indices_within_range(
    values::Vector{<:Real},
    min_value::Real,
    max_value::Real,
)
...
function grid_points_to_basins_in_parallel(
    ...
    for row in eachrow(shape_df)
        ...
        latitude_indices =
            find_indices_within_range(latitudes, min_latitude, max_latitude)
...
function grid_points_to_basins(
    ...
    @showprogress msg pmap(
        grid_points_to_basins_in_parallel_wrapper,
        1:1:num_parts,
    )
end
