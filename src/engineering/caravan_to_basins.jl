...
function caravan_to_basins(
    ...
    for subdir in readdir(attributes_dir)
        ...
        df = CSV.read(
            joinpath(attributes_dir, subdir, "attributes_other_$subdir.csv"),
            DataFrame,
        )
            ...
            if polygon_id != -1
                ...
                if verifies
                    if haskey(map_dict, shape_df[polygon_id, "HYBAS_ID"])
                        if select_best_gauge
                            if catchment_areas[i] >
                               map_dict[shape_df[polygon_id, "HYBAS_ID"]][2]
                                map_dict[shape_df[polygon_id, "HYBAS_ID"]] =
                                    (catchment_ids[i], catchment_areas[i])
                            end
                        else
                            push!(
                                map_dict[shape_df[polygon_id, "HYBAS_ID"]],
